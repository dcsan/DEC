...
export function CostBenefitWidget({ initial, onSend, onRemove }: WidgetProps) {
  ...
  const section = (label: string, rows: CostBenefitLine[], patch: typeof patchCosts, add: () => void, rm: (i: number) => void, gen: () => void) => (
    <div style={{ marginBottom: 12 }}>
      <div style={{ fontSize: 11, fontWeight: 700, marginBottom: 6, color: "var(--vizithink-text-subtle)" }}>{label}</div>
      {rows.map((row, i) => (
  ...
  return (
    <div style={shell(ACCENT)}>
      ...
      <div style={{ padding: "10px 10px 0" }}>
        ...
        <div style={{ fontSize: 11, fontWeight: 700, marginBottom: 6, color: "var(--vizithink-text-subtle)" }}>Notes</div>
        <textarea
          value={notes}
          onChange={(e) => {
        ...
        />
        {error && (
          <div style={{ fontSize: 11, color: "var(--vizithink-option)", marginBottom: 8 }}>{error}</div>
        )}
      </div>
      <div
        style={{
